...
GreaterThan::GreaterThan(int _ID, float _x, float _y, string _name)
{
    ID = _ID;
    x = _x;
    y = _y;
...
void GreaterThan::process()
{
    if(!bProcessed)
    {
        //cout << "process from GreaterThan()\n";
        ...
        if(size0 > 0 && size1 > 0)
        {
            ...
            {
                //cout << it << endl;
                if(boost::get<float>(input[0]->value->data[it % size0]) > boost::get<float>(input[1]->value->data[it % size1]))
                {
